...
# Fetch memory content from GitHub URL directly
def fetch_memory_content():
    url = "https://raw.githubusercontent.com/pritamthakur/ivy-homes-api/refs/heads/main/memory.txt"
    response = requests.get(url)
    return response.text if response.status_code == 200 else "No memory content available."
...
def chat():
    ...
    memory_content = fetch_memory_content()
